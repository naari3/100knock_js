...
var gp_num = 1;
var gp_plot = "";
for (var i = 0; i < 10; i++,gp_num++) {
  gnu_xtics += '"'+word_freqs[i].word+'"'+" "+gp_num+", "  // 「"単語" gnu_num, 」これを追加してる
  // gp_plot += '"'+word_freqs[i].word+'",'+word_freqs[i].count+'),(';
  gp_plot += gp_num+", "+word_freqs[i].count+"\n"
}
...
gp
    ...
    .set('output "plots/037.png"')
    .set('title "37. 頻度上位10語"')
    .set('grid')
    .set('xtics '+gnu_xtics)
    .set('xlabel "単語"')
    .set('ylabel "出現回数"')
    ...
    .plot('"-" with boxes notitle lw 1 lc rgb "light-cyan" \n'+gp_plot)
    .end();
